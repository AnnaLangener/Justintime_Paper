...
# Generating Features
A <- 0.05#1 # Relationship between features and outcome
...
time_type = "None" #DCP, 1-dependent
time_effect = 0
...
sub_id <- unique(features_sample$subject)

auc_value_ind  <- numeric(length(sub_id))
auc_value_base <- numeric(length(sub_id))
sub_number     <- sub_id  # initialize

for (k in seq_along(sub_id)) {

  idx <- features_sample$subject == sub_id[k]
  y_k <- features_sample$y[idx]

  # Only proceed if conditions are met
  if (sum(y_k) > 0 && length(unique(y_k)) > 1) {
    message("Processing subject ", k, " (ID: ", sub_id[k], ")")

    tryCatch(
      {
        res <- run_simulation_slidingwindow(
          features_sample[idx, ],
          1,
          windowsize = 14
        )

        auc_value_ind[k]  <- res$auc_value
        auc_value_base[k] <- res$auc_value_base
      },
      error = function(e) {
        warning("Simulation failed for subject ", sub_id[k], ": ", e$message)
        auc_value_ind[k]  <- NA
        auc_value_base[k] <- NA
      }
    )
...
mean(auc_value_ind, na.rm = TRUE)

sum(!is.na(auc_value_ind))

sum(auc_value_ind[!is.na(auc_value_ind)] != 0)
...
# Table A.1.3
set.seed(12361488)
features_sample <-create_data(n_features, n_samples, n_subjects, A, feature_std = 2, B, C, overall_prob_outcome, sd_outcome, 0, "None")
features_sample <- features_sample[[1]]
...
title = "Bias in Performance Estimates (AUC) When No Relationship Exists: Expected AUC = 0.5"


p1 <- ggplot(result, aes(x=icc, y=auc, color = icc_pred)) + # color: icc_pred
...
result <- read.csv("simulation_results_time_A11.csv") # autoregressive effect in predictors
title = "Robustness Check 1: Bias in Performance Estimates (AUC) When No Relationship Exists: Expected AUC = 0.5"

result <- read.csv("simulation_results_time_A12.csv") # autoregressive effect in predictors
title = "Robustness Check 2: Bias in Performance Estimates (AUC) When No Relationship Exists: Expected AUC = 0.5"

result <- read.csv("simulation_results_time_A13.csv") # autoregressive effect in predictors
result <- result[result$feature_std ==2,]

title = "Robustness Check 3: Bias in Performance Estimates (AUC) When No Relationship Exists: Expected AUC = 0.5"
...
plot_data = function(data_viz, spanpar){
  ...
  p = ggplot(df_long, aes(x = icc_data, y = AUC, color = AUC_type)) +
    ...
    scale_color_manual(
      values = c(
        "shuffle_AUC." = "#c7835a",
        "movingwindowshuffle_AUC." = "#58538f"
      ), 
      labels = c(
...
plot_data = function(data_viz, spanpar){
  ...
  p = ggplot(df_long, aes(x = icc_data, y = Correlation, color = type)) +
    ...
    scale_color_manual(values = c(
      "shuffle_cor" = "#c7835a",
      "movingwindowshuffle_cor" = "#58538f"
...
p2 <- ggplot(data, aes(x = time, y = y)) +
  ...
  geom_point(color = "#c7835a", size = 0.5) +                       
  facet_wrap(~subject) +               
  labs(
    x = "Time", 
    y = "Outcome", 
...
############ Table 7 #############
##################################
round(sim_1$auc_value_base,2) # row-wise
round(sim_4$auc_value_base,2) # rolling window
sim_3$auc_value_base # subject-wise
...
plot_data = function(data_viz,spanpar){
  ...
  p = ggplot(df_long, aes(x = icc_data, y = AUC, color = AUC_type)) +
    ...
    theme_minimal() +
    scale_color_manual(values = c(
      "real_AUC.random.intercept.only." ="#c7835a" ,
      "real_AUC." = "#9c5435",
      "movingwindow_AUC." ="#877fbf",
      "movingwindow_AUC.random.intercept.only." = "#58538f",
      "subjectwise_AUC." ="#40916c"
    ), 
    labels = c(
...
plot_data = function(data_viz,spanpar){
  ...
  p = ggplot(df_long, aes(x = icc_data, y = Correlation, color = type)) +
    ...
    theme_minimal() +
    scale_color_manual(values = c(
      "real_cor_base" ="#c7835a" ,
      "real_cor" = "#9c5435",
      "movingwindow_cor" = "#877fbf",
      "movingwindow_cor_base" = "#58538f",
      "subjectwise_cor" ="#40916c"
...
title = paste("Performance of a Random Intercept Only Model (No Predictors Included)")


result = read.csv("simulation_results_baseline_A21.csv")
title = paste("Robustness Check 1: Performance of a Random Intercept Only Model (No Predictors Included)")



result = read.csv("simulation_results_baseline_A22.csv")
title = paste("Robustness Check 2: Performance of a Random Intercept Only Model (No Predictors Included)")
...
```{r }
# --- PARAMETERS ---
a_values <- c(0.00, 0.05)        # outcome effect
b_values <- c(0.1, 0.5, 0.9)     # cross-subject variability
c_values <- c(0.05, 0.25, 0.45)  # within-subject variability
d <- 0.1                         # noise

# --- COMPUTE SNR ---
results <- expand.grid(a = a_values, b = b_values, c = c_values)

results$var_signal <- results$a^2
results$var_noise  <- results$b^2 + results$c^2 + d^2
results$SNR_linear <- results$var_signal / results$var_noise
results$SNR_dB     <- ifelse(results$SNR_linear > 0,
                             10 * log10(results$SNR_linear),
                             -Inf)

# --- SHOW RESULTS ---
print(round(results, 6))

b_grid <- seq(0.1, 0.9, by = 0.05)
c_grid <- seq(0.05, 0.45, by = 0.05)
a <- 0.05

grid <- expand.grid(b = b_grid, c = c_grid)
grid$SNR_linear <- a^2 / (grid$b^2 + grid$c^2 + d^2)
grid$SNR_dB <- 10 * log10(grid$SNR_linear)

summary(grid$SNR_linear)
summary(grid$SNR_dB)

# Optional: visualize as heatmap
library(ggplot2)
ggplot(grid, aes(x = b, y = c, fill = SNR_linear)) +
  geom_tile(color = "white") +
  scale_fill_viridis_c(option = "plasma") +
  labs(title = "SNR (dB) across b and c values",
       x = "Cross-subject variability (b)",
       y = "Within-subject variability (c)",
       fill = "SNR (dB)") +
...
result <- read.csv("signal_to_noise.csv")
mean(result$R2_marginal)
median(result$R2_marginal)
min(result$R2_marginal)
max(result$R2_marginal)
