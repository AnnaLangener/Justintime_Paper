...
cols_to_select1 <- c("outcome", "overall_prob_outcome", "sd_outcome",
                  "icc_data")
cols_to_select <- c("overall_prob_outcome", "sd_outcome",
                   "icc_data")

df2_selected <- final_results_df_2_b[, cols_to_select1]
df3_selected <- final_results_df_3_b[, cols_to_select]
df4_selected <- final_results_df_4_b[, cols_to_select]
df5_selected <- final_results_df_5_b[, cols_to_select]

# Rename columns to distinguish between dataframes (optional but recommended)
names(df2_selected) <- paste0(names(df2_selected), "_df2")
names(df3_selected) <- paste0(names(df3_selected), "_df3")
names(df4_selected) <- paste0(names(df4_selected), "_df4")
names(df5_selected) <- paste0(names(df5_selected), "_df5")

# Combine the selected dataframes column-wise
combined_df <- cbind(df2_selected, df3_selected, df4_selected, df5_selected)

# View or write the combined data frame
head(combined_df)

combined_df <- combined_df %>%
  mutate(across(where(is.numeric), ~ round(.x, 2)))
write.csv(combined_df, "combined_des_binary.csv", row.names = FALSE)
...
df2_selected <- final_results_df_2[, cols_to_select1]
df3_selected <- final_results_df_3[, cols_to_select]
df4_selected <- final_results_df_4[, cols_to_select]
df5_selected <- final_results_df_5[, cols_to_select]
...
combined_df <- combined_df %>%
  mutate(across(where(is.numeric), ~ round(.x, 2)))
write.csv(combined_df, "combined_des_continious.csv", row.names = FALSE)
